...
const CuponesTab = ({ triggerCreate }) => {
  ...
  const [form, setForm] = useState({ codigo: '', tipo: 'porcentaje', valor: '', descripcion: '', fecha_inicio: '', fecha_fin: '', limite_usos: '', activo: true });
  ...
  // Escuchar trigger externo del botón del header
  ...
  const openEdit = (c) => {
    setEditando(c);
    setForm({ codigo: c.codigo, tipo: c.tipo, valor: c.valor, descripcion: c.descripcion || '', activo: c.activo, fecha_inicio: c.fecha_inicio ? c.fecha_inicio.slice(0,10) : '', fecha_fin: c.fecha_fin ? c.fecha_fin.slice(0,10) : '', limite_usos: c.limite_usos || '' });
    setError('');
    setModalOpen(true);
  };
  ...
  const stats = { total: cupones.length, activos: cupones.filter(c => c.activo).length, inactivos: cupones.filter(c => !c.activo).length };
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4">
        ...
        <p className="text-xs text-gray-400 mt-3">Mostrando <span className="font-semibold text-gray-600">{cupones.length}</span> de <span className="font-semibold text-gray-600">{total}</span> cupones</p>
      </div>
        ...
        {loading ? (
          ...
          <div className="divide-y divide-gray-50">
            <div className="hidden sm:grid grid-cols-12 gap-4 px-5 py-3 bg-gray-50 text-xs font-semibold text-gray-400 uppercase tracking-wider">
              <div className="col-span-2">Código</div><div className="col-span-2">Tipo</div><div className="col-span-2">Valor</div>
              <div className="col-span-2">Usos</div><div className="col-span-2">Vence</div><div className="col-span-1">Estado</div><div className="col-span-1 text-right">Acc.</div>
            </div>
            {cupones.map(c => (
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={form.codigo} onChange={e => setForm(p => ({ ...p, codigo: e.target.value.toUpperCase() }))}
              placeholder="Ej: DESCUENTO20" className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-purple-300" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="number" min="0" max={form.tipo === 'porcentaje' ? 100 : undefined} value={form.valor}
                onChange={e => setForm(p => ({ ...p, valor: e.target.value }))} placeholder={form.tipo === 'porcentaje' ? 'Ej: 20' : 'Ej: 5000'}
                className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300" />
            </div>
          </div>
          <div>
            ...
            <textarea value={form.descripcion} onChange={e => setForm(p => ({ ...p, descripcion: e.target.value }))} rows={2}
              placeholder="Descripción opcional" className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300 resize-none" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
          ...
          <div>
            ...
            <input type="number" min="1" value={form.limite_usos} onChange={e => setForm(p => ({ ...p, limite_usos: e.target.value }))}
              placeholder="Vacío = ilimitado" className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300" />
          </div>
      ...
      <Modal open={!!deleteModal} onClose={() => setDeleteModal(null)} title="Eliminar cupón">
        <p className="text-gray-600 text-sm mb-6">¿Eliminar el cupón <span className="font-mono font-bold text-purple-700">{deleteModal?.codigo}</span>? No se puede deshacer.</p>
...
const PromocionesTab = ({ triggerCreate }) => {
  ...
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const [form, setForm] = useState({ nombre: '', descripcion: '', tipo_descuento: 'porcentaje', valor_descuento: '', fecha_inicio: '', fecha_fin: '', activa: true });
  ...
  const openEdit = (p) => {
    setEditando(p);
    setForm({ nombre: p.nombre, descripcion: p.descripcion || '', tipo_descuento: p.tipo_descuento, valor_descuento: p.valor_descuento, activa: p.activa, fecha_inicio: p.fecha_inicio ? p.fecha_inicio.slice(0,10) : '', fecha_fin: p.fecha_fin ? p.fecha_fin.slice(0,10) : '' });
    setError('');
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const body = { ...form, valor_descuento: parseFloat(form.valor_descuento) };
  ...
  const stats = { total: promociones.length, activas: promociones.filter(p => p.activa).length, inactivas: promociones.filter(p => !p.activa).length };
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4">
        ...
        <p className="text-xs text-gray-400 mt-3">Mostrando <span className="font-semibold text-gray-600">{promociones.length}</span> de <span className="font-semibold text-gray-600">{total}</span> promociones</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="divide-y divide-gray-50">
            <div className="hidden sm:grid grid-cols-12 gap-4 px-5 py-3 bg-gray-50 text-xs font-semibold text-gray-400 uppercase tracking-wider">
              <div className="col-span-3">Nombre</div><div className="col-span-2">Descuento</div><div className="col-span-2">Inicio</div>
              <div className="col-span-2">Fin</div><div className="col-span-2">Estado</div><div className="col-span-1 text-right">Acc.</div>
            </div>
            {promociones.map(p => (
              <div key={p.id} className="grid grid-cols-12 gap-4 px-5 py-4 items-center hover:bg-gray-50 transition-colors">
                ...
                <div className="col-span-2"><span className="font-semibold text-sm text-purple-700 bg-purple-50 px-2 py-1 rounded">{p.tipo_descuento === 'porcentaje' ? `${p.valor_descuento}%` : `$${Number(p.valor_descuento).toLocaleString()}`}</span></div>
                <div className="col-span-2 text-sm text-gray-500">{p.fecha_inicio ? new Date(p.fecha_inicio).toLocaleDateString('es-ES') : '—'}</div>
                <div className="col-span-2 text-sm text-gray-500">{p.fecha_fin ? new Date(p.fecha_fin).toLocaleDateString('es-ES') : '—'}</div>
                <div className="col-span-2"><Badge active={isVigente(p)} label={isVigente(p) ? 'Vigente' : p.activa ? 'Fuera de fecha' : 'Inactiva'} /></div>
                <div className="col-span-1 flex justify-end gap-1">
                  <button onClick={() => openEdit(p)} className="p-1.5 hover:bg-blue-50 rounded-lg"><svg className="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
                  <button onClick={() => setDeleteModal(p)} className="p-1.5 hover:bg-red-50 rounded-lg"><svg className="w-4 h-4 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={form.nombre} onChange={e => setForm(p => ({ ...p, nombre: e.target.value }))}
              placeholder="Ej: Black Friday 2025" className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Descripción</label>
            <textarea value={form.descripcion} onChange={e => setForm(p => ({ ...p, descripcion: e.target.value }))} rows={2}
              placeholder="Descripción de la promoción" className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300 resize-none" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
      ...
      <Modal open={!!deleteModal} onClose={() => setDeleteModal(null)} title="Eliminar promoción">
        <p className="text-gray-600 text-sm mb-6">¿Eliminar la promoción <span className="font-bold text-gray-900">"{deleteModal?.nombre}"</span>? No se puede deshacer.</p>
        <div className="flex gap-3">
          <button onClick={() => setDeleteModal(null)} className="flex-1 border border-gray-200 text-gray-600 py-2.5 rounded-lg text-sm font-medium hover:bg-gray-50">Cancelar</button>
          <button onClick={handleDelete} className="flex-1 bg-red-500 text-white py-2.5 rounded-lg text-sm font-medium hover:bg-red-600">Eliminar</button>
        </div>
